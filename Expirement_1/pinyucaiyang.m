...
% 条件 1：0 <= n <= 13 时，x(n) = n + 1
idx1=(n>=0)&(n<=13);
x(idx1)=n(idx1)+1;

% 条件 2：14 <= n <= 26 时，x(n) = 27 - n
idx2=(n>=14)&(n<=26);
x(idx2)=27-n(idx2);

% ----------------- 2. 补零到 32 点，做 32 点 DFT -----------------
% 把长度 27 的序列补 0 到长度 32，方便做 32 点 FFT
...
% 由 32 点频谱还原的序列 x_32(n)
subplot(3,1,2);
stem(0:31,real(x32_t));
xlabel('n');
ylabel('x_{32}(n)');
title('由32点频谱还原的 x_{32}(n)');

% 由 16 点频谱还原的序列 x_16(n)
subplot(3,1,3);
stem(0:15,real(x16_t));
xlabel('n');
ylabel('x_{16}(n)');
